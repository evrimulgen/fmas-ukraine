...
uses
  ...
  cxLookAndFeelPainters, cxButtons, frxDesgn;
...
begin
  ...
  DataSetSel.SelectSQL.Text:='select * from dog_ree_fnzb_select(:DATE_BEG,:DATE_END,:REG_DOG,:NUM_REE,:NUM_DOG)';
  DataSetSel.ParamByName('DATE_BEG').AsDate:=DateBeg.Date;
  DataSetSel.ParamByName('DATE_END').AsDate:=DateEnd.Date;

  if (fltForm1.cxCheckBox1.Checked) then
    DataSetSel.ParamByName('REG_DOG').AsString:=fltForm1.RegNumEdit.Text
  else
    DataSetSel.ParamByName('REG_DOG').AsString:='';

  if (fltForm1.cxCheckBox2.Checked) then
    DataSetSel.ParamByName('NUM_REE').AsString:=fltForm1.NumEdit.Text
  else
    DataSetSel.ParamByName('NUM_REE').AsString:='';

  if (fltForm1.cxCheckBoxNumDog.Checked) then
    DataSetSel.ParamByName('NUM_DOG').AsString:=fltForm1.cxTextEditNumDog.Text
  else
    DataSetSel.ParamByName('NUM_DOG').AsString:='';

  DataSetSel.Active := true;
  fltForm1.RegNumEdit.Text:='';
  fltForm1.NumEdit.Text:='';
  fltForm1.cxTextEditNumDog.Text:='';
...
begin
 ...
 for k := 0 to Length(id_reestr) - 1 do begin
  ...
  begin
    ...
    DbfExportFn['FLAGS']    := DataSetExportFz.FBN('IS_INFO_PRO_OBM_STR').AsString;
