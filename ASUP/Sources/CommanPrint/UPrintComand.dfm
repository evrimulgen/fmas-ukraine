object fmPrintComand: TfmPrintComand
  Left = 263
  Top = 267
  BorderStyle = bsDialog
  Caption = #1044#1088#1091#1082
  ClientHeight = 86
  ClientWidth = 424
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 3
    Top = 20
    Width = 103
    Height = 13
    Caption = #1058#1080#1087' '#1092#1086#1088#1084#1080' '#1076#1088#1091#1082#1091
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object PrintButton: TcxButton
    Left = 80
    Top = 48
    Width = 120
    Height = 28
    Caption = #1044#1088#1091#1082#1091#1074#1072#1090#1080
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    OnClick = PrintButtonClick
    Glyph.Data = {
      
      000000000000000000000000000032323200B6B6B600FFFFFF00}
    UseSystemPaint = False
  end
  object CancelButton: TcxButton
    Left = 264
    Top = 48
    Width = 120
    Height = 28
    Caption = #1042'i'#1076#1084'i'#1085#1080#1090#1080
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    OnClick = CancelButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFF0000}
    UseSystemPaint = False
  end
  object cxComboBoxSelComand: TcxComboBox
    Left = 112
    Top = 16
    Width = 297
    Height = 21
    Properties.Items.Strings = (
      #1055#1086#1089#1074'i'#1076#1095#1077#1085#1085#1103' '#1087#1088#1086' '#1074'i'#1076#1088#1103#1076#1078#1077#1085#1085#1103
      #1053#1072#1082#1072#1079
      #1047#1074'i'#1090' '#1087#1088#1086' '#1074'i'#1076#1088#1103#1076#1078#1077#1085#1085#1103)
    TabOrder = 2
    Text = 'cxComboBoxSelComand'
  end
  object pFIBDatabaseCommand: TpFIBDatabase
    DBName = 'D:\Hunter\Database_asup\FULL_DB.IB'
    DBParams.Strings = (
      'lc_ctype=win1251'
      'password=masterkey'
      'user_name=SYSDBA')
    DefaultTransaction = TransactionR
    SQLDialect = 3
    Timeout = 0
    SaveDBParams = False
    WaitForRestoreConnect = 0
    Left = 8
    Top = 40
  end
  object DataSetSelPI: TpFIBDataSet
    Database = pFIBDatabaseCommand
    Transaction = TransactionR
    SelectSQL.Strings = (
      'select * from ASUP_COMAND_PRINT_F(15063612)')
    Left = 40
    Top = 40
    poSQLINT64ToBCD = True
  end
  object TransactionR: TpFIBTransaction
    DefaultDatabase = pFIBDatabaseCommand
    TimeoutAction = TARollback
    Left = 72
    Top = 40
  end
  object frxReportPrint: TfrxReport
    Version = '3.15'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    ReportOptions.CreateDate = 38762.531632662000000000
    ReportOptions.LastChange = 38762.558108171300000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 232
    Top = 48
    Datasets = <
      item
        DataSet = frxDBDatasetSablon
        DataSetName = 'frxDBDatasetSablon'
      end>
    Variables = <>
    Style = <>
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 210.000000000000000000
      PaperHeight = 148.000000000000000000
      PaperSize = 11
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle1: TfrxReportTitle
        Height = 464.882190000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        object Memo1: TfrxMemoView
          Top = 3.779530000000001000
          Width = 718.110700000000000000
          Height = 26.456710000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -24
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.Strings = (
            #1053#1040#1050#1040#1047)
          ParentFont = False
        end
        object Memo2: TfrxMemoView
          Top = 37.795300000000000000
          Width = 718.110700000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.Strings = (
            
              #1082#1088#1072#1111#1085#1072' '#1074#1080#1088#1086#1073#1085#1080#1095#1086#1075#1086' '#1090#1086#1074#1072#1088#1089#1090#1074#1072' '#1087#1086' '#1074#1086#1076#1086#1079#1072#1073#1077#1089#1087#1077#1095#1077#1085#1085#1102' '#1044#1086#1085#1073#1072#1089#1072' '#1080' "'#1059#1082#1088#1087 +
              #1088#1086#1084#1074#1086#1076#1095#1077#1088#1084#1077#1090'"')
          ParentFont = False
        end
        object Memo3: TfrxMemoView
          Top = 68.031540000000010000
          Width = 718.110700000000000000
          Height = 18.897650000000000000
          HAlign = haCenter
          Memo.Strings = (
            
              '         [frxDBDatasetSablon."DATE_ORD"]   '#1088#1086#1082#1091'                 ' +
              '                                                       '#1084'. '#1044#1086#1085#1077#1094#1100 +
              #1082)
        end
        object Memo4: TfrxMemoView
          Left = 226.771800000000000000
          Top = 94.488250000000000000
          Width = 26.456710000000000000
          Height = 22.677180000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.Strings = (
            #8470)
          ParentFont = False
        end
        object Memo5: TfrxMemoView
          Top = 120.944960000000000000
          Width = 71.811070000000000000
          Height = 18.897650000000000000
          Memo.Strings = (
            #1042#1110#1076#1088#1103#1076#1080#1090#1080)
        end
        object Memo6: TfrxMemoView
          Left = 75.590600000000000000
          Top = 120.944960000000000000
          Width = 642.520100000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.Strings = (
            '[frxDBDatasetSablon."FIO"]')
          ParentFont = False
        end
        object Memo7: TfrxMemoView
          Left = 71.811070000000000000
          Top = 139.842610000000000000
          Width = 646.299630000000000000
          Height = 18.897650000000000000
          HAlign = haCenter
          Memo.Strings = (
            #1087#1088#1110#1079#1074#1080#1097#1077', '#1110#1084#39#1103', '#1087#1086#1073#1072#1090#1100#1082#1086#1074#1110)
        end
        object Memo8: TfrxMemoView
          Left = 3.779530000000000000
          Top = 162.519790000000000000
          Width = 151.181200000000000000
          Height = 18.897650000000000000
          Memo.Strings = (
            #1052#1110#1089#1094#1077' '#1088#1086#1073#1086#1090#1080', '#1087#1086#1089#1072#1076#1072)
        end
        object Memo9: TfrxMemoView
          Left = 158.740260000000000000
          Top = 162.519790000000000000
          Width = 563.149970000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.Strings = (
            '[frxDBDatasetSablon."POSADA"]')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Top = 192.756030000000000000
          Width = 718.110700000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          Top = 222.992270000000000000
          Width = 147.401670000000000000
          Height = 18.897650000000000000
          Memo.Strings = (
            #1050#1091#1076#1080' '#1074#1110#1076#1088#1103#1076#1080#1090#1080)
        end
        object Memo12: TfrxMemoView
          Left = 147.401670000000000000
          Top = 222.992270000000000000
          Width = 570.709030000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.Strings = (
            '[frxDBDatasetSablon."PLACE_TARGET_WITH_TARGET"]')
          ParentFont = False
        end
        object Memo13: TfrxMemoView
          Top = 253.228510000000000000
          Width = 147.401670000000000000
          Height = 18.897650000000000000
          Memo.Strings = (
            #1052#1077#1090#1072)
        end
        object Memo14: TfrxMemoView
          Left = 147.401670000000000000
          Top = 253.228510000000000000
          Width = 570.709030000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.Strings = (
            '[frxDBDatasetSablon."TARGET"]')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          Left = 0.000000000000000444
          Top = 283.464750000000000000
          Width = 718.110700000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          Top = 313.700990000000000000
          Width = 147.401670000000000000
          Height = 18.897650000000000000
          Memo.Strings = (
            #1058#1077#1088#1084#1110#1085' '#1074#1110#1076#1088#1103#1076#1078#1077#1085#1085#1103)
        end
        object Memo17: TfrxMemoView
          Left = 147.401670000000000000
          Top = 313.700990000000000000
          Width = 570.709030000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.Strings = (
            '[frxDBDatasetSablon."TERMIN"] '#1076#1110#1073)
          ParentFont = False
        end
        object Memo18: TfrxMemoView
          Top = 343.937230000000000000
          Width = 147.401670000000000000
          Height = 18.897650000000000000
          Memo.Strings = (
            #1055#1110#1076#1089#1090#1072#1074#1072)
        end
        object Memo19: TfrxMemoView
          Left = 147.401670000000000000
          Top = 343.937230000000000000
          Width = 570.709030000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.Strings = (
            '[frxDBDatasetSablon."OSNOVANIE"]')
          ParentFont = False
        end
        object Memo20: TfrxMemoView
          Top = 374.173470000000000000
          Width = 147.401670000000000000
          Height = 18.897650000000000000
          Memo.Strings = (
            #1055#1088#1086#1111#1079#1076)
        end
        object Memo21: TfrxMemoView
          Left = 147.401670000000000000
          Top = 374.173470000000000000
          Width = 570.709030000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.Strings = (
            '[frxDBDatasetSablon."PROEZD"]')
          ParentFont = False
        end
        object Memo22: TfrxMemoView
          Top = 404.409710000000000000
          Width = 718.110700000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.Strings = (
            
              #1047#1072#1090#1074#1077#1088#1076#1078#1077#1085#1086': '#1050#1080#1088#1110#1074#1085#1080#1082' '#1074#1110#1076#1076#1110#1083#1091' _____________________ '#1043#1086#1083#1086#1074#1085#1080#1081' '#1073#1091#1093 +
              #1075#1072#1083#1090#1077#1088' _____________________')
          ParentFont = False
        end
        object Memo23: TfrxMemoView
          Left = 90.708720000000000000
          Top = 430.866420000000000000
          Width = 627.401980000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.Strings = (
            
              #1043#1077#1085#1077#1088#1072#1083#1100#1085#1080#1081' '#1076#1080#1088#1077#1082#1090#1086#1088'  __________________________________________' +
              '______________')
          ParentFont = False
        end
        object Memo24: TfrxMemoView
          Left = 257.008040000000000000
          Top = 94.488250000000000000
          Width = 238.110390000000000000
          Height = 18.897650000000000000
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.Strings = (
            '[frxDBDatasetSablon."NUM_ORDER"]')
        end
      end
    end
  end
  object frxDBDatasetSablon: TfrxDBDataset
    UserName = 'frxDBDatasetSablon'
    CloseDataSource = False
    DataSet = DataSetSelPI
    Left = 192
    Top = 48
  end
end
