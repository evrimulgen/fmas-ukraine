...
type
  ...
  //  ResultArray: TAArray;
   ...
   // PBForm: TForm_cs_Orders_ProgressBar;
    LogFile: TextFile;
    bWriteLogToFile: Boolean;
    procedure ExceptionProcessing(storProc: TpFibStoredProc; exceptionMessage: string);
...
{$R *.dfm}
procedure TfmStudImportInfo.ExceptionProcessing(storProc: TpFibStoredProc; exceptionMessage: string);
begin
    if bWriteLogToFile then
    begin
        Append(logFile);
        Writeln(logFile, storProc.ReadySQLText);
        Writeln(logFile, exceptionMessage);
        Writeln(logFile, '');
        CloseFile(logFile);
    end
    else
        csMessageDlg(nMsgBoxTitle[IndLangVWF], nMsgErrorWithEDBO[IndLangVWF] + #10#13 + exceptionMessage, mtInformation, [mbOk], IndLangVWF);
...
var
    ...
    UseFilter: Boolean;
    FIOMask,DocumentSeries,DocumentNumber :string;
...
begin
    ...
    if (EDBOPersonIntf as IEDBOTools).InitEDBOConnection('EDBOPerson', DM.DB.Handle) then
    begin
        bWriteLogToFile := false;
        dlgSaveLog := TSaveDialog.Create(Self);
        try
            dlgSaveLog.DefaultExt := 'txt';
            if dlgSaveLog.Execute then
            begin
                bWriteLogToFile := true;
                AssignFile(LogFile, dlgSaveLog.FileName);
            end;
        finally
            dlgSaveLog.Free;
        end;
...
begin
    ...
    try
        ...
        begin
            ExceptionProcessing(DM.StorProc, E.Message);
            Result := False;
            exit;
        end;
    end;
    ...
    try
        ...
        try
            ...
            begin
                ...
                ExceptionProcessing(StorProcDelEduc, E.Message);
                Result := False;
            end;
        end;
        ...
        begin
            ...
            try
                ...
                begin
                    ExceptionProcessing(StorProcInfoInsert, E.Message);
                    Result := False;
                    exit;
                end;
            end;
...
begin
    ...
    try
        ...
        do begin
            ExceptionProcessing(DM.SyncStorProc, E.Message);
            Result := False;
        end;
    end;
end;
