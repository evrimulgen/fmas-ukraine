...
begin
    DB.Handle := PDB_Handle;
    IdServerDSet.Close;
    IdServerDSet.SQLs.SelectSQL.Text:='select Id_Server from pub_sys_data';
    IdServerDSet.Open;
    if not (VarIsNull(IdServerDSet['Id_Server'])) then
    IdServer := IdServerDSet['Id_Server']
    else IdServer := 99;
    //IdServer=91 - ХАИ Студгородок и ХАИ
    sFolder:= '';
    bool:=false;
    if IdServer = 91 then
    begin
        bool:=SaveDialog1.Execute;
        if bool then FilePath.Text:=SaveDialog1.Path;
    end
    else
    begin
        bool:=GetFolderDialog(Application.Handle, '', sFolder);
        if bool then FilePath.Text:=sFolder;
    end;
    ...
    begin
        ...
        if((Y>=2015) and (M>=5))then
        begin
          DbfExportT5.TableName:='E04T05I.dbf ';
          DbfExportT6.TableName:='E04T06I.dbf';
          DbfExportT7.TableName:='E04T07I.dbf';
          DbfExportT8.TableName:='E04T08I.dbf';
          DbfExportT9.TableName:='E04T09I.dbf';
        end
        else
        begin
