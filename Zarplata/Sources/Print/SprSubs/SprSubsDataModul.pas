...
var IniFile:TIniFile;
    ...
    PSumNALNSTIP:Extended;
    PSumNALSTIP:Extended;
    s:string;
begin
 ...
 while not DSet.Eof do
  begin
   ...
   PSumNALNSTIP:=PSumNALNSTIP+DSet['SUM_NALNSTIP'];
   PSumNALSTIP:=PSumNALSTIP+DSet['SUM_NALSTIP'];
   DSet.Next;
  end;
 ...
 if PSumNALNSTIP=0 then
  Report.Variables.AddVariable('User Category','PSumNALNSTIP','''----- ''')
 else
  Report.Variables.AddVariable('User Category','PSumNALNSTIP',''''+FloatToStrF(PSumNALNSTIP,ffFixed,16,2)+' ''');

  if PSumNALSTIP=0 then
  Report.Variables.AddVariable('User Category','PSumNALSTIP','''----- ''')
 else
  Report.Variables.AddVariable('User Category','PSumNALSTIP',''''+FloatToStrF(PSumNALSTIP,ffFixed,16,2)+' ''');
 ...
 Report.Variables.AddVariable('User Category','PYear',''''+IntToStr(YearMonthFromKodSetup(AParameter.Kod_setup1,true))+'''');


 case ViewMode of
   1: Report.ShowReport;
   2: Report.ShowReport; //Report.DesignReport;
  end;
 Report.Free;
end;
...
if UpperCase(VarName)='PKOD_SETUP' then
  ...
  Value:=YearMonthFromKodSetup(DSet['KOD_SETUP'],true);
...
if UpperCase(VarName)='PSUM_MONEY' then
  ...
  if UpperCase(VarName)='PSUM_NALNSTIP' then
  begin
   if (DSet['SUM_NALNSTIP']=NULL) or (DSet['SUM_NALNSTIP']=0) then
    value:='----- '
   else
    Value:=FloatToStrF(DSet['SUM_NALNSTIP'],ffFixed,16,2)+' ';
  end;
...
if UpperCase(VarName)='PSUM_INSHI' then
  ...
  if UpperCase(VarName)='PSUM_NALSTIP' then
  begin
   if (DSet['SUM_NALSTIP']=NULL) or (DSet['SUM_NALSTIP']=0) then
    value:='----- '
   else
    Value:=FloatToStrF(DSet['SUM_NALSTIP'],ffFixed,16,2)+' ';
  end;
