...
uses
  ...
  st_ConstUnit, st_common_funcs, DateUtils;
...
type
  ...
  public
    ...
    id_type_stud : Int64;
    id_category  : Int64;
    id_price, id_price_param     : Int64;
    DATE_BEG_PR, DATE_END_PR : TDate;
    DM           : Tfrm_price_DM;
    is_add       : Boolean;
    id_kat_pay   : Int64;
...
begin
  ...
  if ViewForm.ModalResult = mrOK then
   Begin
    ...
    DM.DataSet_main.SQLs.SelectSQL.Text := 'select * from ST_GET_PRICE_KAT(:ID_BUILD, :ID_SERVICE, :ID_TYPE_NORMA, :S_ROOM, :KOL_LIVE, :ID_OPTIONS, :DATE_BEG_PRICE, :DATE_END_PRICE) order by ST_OPTIONS desc';
    DM.DataSet_main.ParamByName('ID_BUILD').AsInt64 := Id_Builds;
    DM.DataSet_main.ParamByName('ID_SERVICE').AsInt64 := ViewForm.id_serves;
    DM.DataSet_main.ParamByName('ID_TYPE_NORMA').AsInt64 := ViewForm.ComboBox_type_norm.ItemIndex + 1;

    DM.DataSet_main.ParamByName('S_ROOM').AsDouble := S_room;
    DM.DataSet_main.ParamByName('KOL_LIVE').AsInteger := kol_live;
    DM.DataSet_main.ParamByName('ID_OPTIONS').AsInteger := ViewForm.id_options;

    DM.DataSet_main.ParamByName('DATE_BEG_PRICE').AsDate := DATE_BEG_PR;
    DM.DataSet_main.ParamByName('DATE_END_PRICE').AsDate := DATE_END_PR;//ID_PRICE;
    ...
    for i := 0 to Dm.DataSet_main.RecordCount - 1 do
     Begin
         ...
         DM.MemoryData_services['MemoryData_tariff']          := DM.DataSet_main['TARIF'];
         DM.MemoryData_services['MemoryData_norma']           := DM.DataSet_main['NORMA'];

         //DM.MemoryData_services['MemoryData_norma']:=formatfloat('0.00', DM.MemoryData_services['MemoryData_norma']);

         DM.MemoryData_services['MemoryData_summa']       := DM.DataSet_main['SUMMA'];
         ...
         DM.MemoryData_services['MemoryData_date_beg']  := DM.DataSet_main['DATE_BEG'];
         DM.MemoryData_services['MemoryData_date_end']  := DM.DataSet_main['DATE_END'];
...
begin
  ...
  ViewForm.date_beg               := DM.MemoryData_services['MemoryData_date_beg'];
  ViewForm.date_end               := DM.MemoryData_services['MemoryData_date_end'];
  ...
  if ViewForm.ModalResult = mrOK then
   Begin
    ...
    DM.DataSet_main.ParamByName('ID_OPTIONS').AsInteger := ViewForm.id_options;
    DM.DataSet_main.ParamByName('DATE_BEG_PRICE').AsDate := DATE_BEG_PR;
    DM.DataSet_main.ParamByName('DATE_END_PRICE').AsDate := DATE_END_PR;
...
var
  i, j, id_options, id_sm, id_kekv, id_type_norma, id_st : Integer;
  id_services : int64;
  date_beg, date_end: TDate;
  name_type_norma, name, sm_kod, kekv_kod: String;
...
begin
  ...
  For i := 0 to DM.MemoryData_services.RecordCount - 1 do
   Begin
    ...
    id_type_norma          := DM.MemoryData_services['MemoryData_id_type_norma'];
    name_type_norma          := DM.MemoryData_services['MemoryData_name_type_norma'];
    id_options             := DM.MemoryData_services['MemoryData_st_options'];
    name                    := DM.MemoryData_services['MemoryData_name'];
    id_sm          := DM.MemoryData_services['MemoryData_id_sm'];
    sm_kod        := DM.MemoryData_services['MemoryData_sm_tittle'];
    kekv_kod        := DM.MemoryData_services['MemoryData_KEKV_tittle'];
    id_kekv        := DM.MemoryData_services['MemoryData_id_KEKV'];
    id_st         := DM.MemoryData_services['MemoryData_id_st'];

    DM.DataSet_read.close;
    DM.DataSet_read.SQLs.SelectSQL.Text := 'select * from ST_GET_PRICE_KAT(:ID_BUILD, :ID_SERVICE, :ID_TYPE_NORMA, :S_ROOM, :KOL_LIVE, :ID_OPTIONS, :DATE_BEG_PRICE, :DATE_END_PRICE) where ID_STAT = :ID_STAT order by ST_OPTIONS desc';
    DM.DataSet_read.ParamByName('ID_BUILD').AsInt64 := Id_Builds;
    ...
    DM.DataSet_read.ParamByName('S_ROOM').AsDouble := S_room;
    DM.DataSet_read.ParamByName('KOL_LIVE').AsInteger := kol_live;
    DM.DataSet_read.ParamByName('ID_OPTIONS').AsInteger := DM.MemoryData_services['MemoryData_st_options'];

    DM.DataSet_read.ParamByName('DATE_BEG_PRICE').AsDate := DM.MemoryData_services['MemoryData_date_beg'];
    DM.DataSet_read.ParamByName('DATE_END_PRICE').AsDate := DM.MemoryData_services['MemoryData_date_end'];
    DM.DataSet_read.ParamByName('ID_STAT').AsInteger := id_st;

    DM.DataSet_read.Open;
    DM.DataSet_read.FetchAll;


    DM.MemoryData_services.Delete;
    //DM.MemoryData_services.Next;
    ...
    Begin
        ...
        for j := 0 to Dm.DataSet_read.RecordCount - 1 do
         Begin
             DM.MemoryData_services.Insert;
             DM.MemoryData_services['MemoryData_id']              := id_services;
             DM.MemoryData_services['MemoryData_id_type_norma']   := id_type_norma;
             DM.MemoryData_services['MemoryData_name_type_norma'] := name_type_norma;
             DM.MemoryData_services['MemoryData_name']            := name;
             DM.MemoryData_services['MemoryData_buget']           := sm_kod + '.' + IntToStr(DM.DataSet_read['KOD_RAZD']) + '.' + IntToStr(DM.DataSet_read['KOD_STAT']) + '.' + kekv_kod;
             DM.MemoryData_services['MemoryData_tariff']          := DM.DataSet_read['TARIF'];
             DM.MemoryData_services['MemoryData_norma']           := DM.DataSet_read['NORMA'];

             //DM.MemoryData_services['MemoryData_norma']:=formatfloat('0.00', DM.MemoryData_services['MemoryData_norma']);

             DM.MemoryData_services['MemoryData_summa']       := DM.DataSet_read['SUMMA'];
             DM.MemoryData_services['MemoryData_id_sm']       := id_sm;
             DM.MemoryData_services['MemoryData_Id_rz']       := DM.DataSet_read['ID_RAZD'];
             DM.MemoryData_services['MemoryData_id_st']       := DM.DataSet_read['ID_STAT'];
             DM.MemoryData_services['MemoryData_id_KEKV']     := id_kekv;
             DM.MemoryData_services['MemoryData_sm_tittle']   := sm_kod;
             DM.MemoryData_services['MemoryData_rz_tittle']   := DM.DataSet_read['KOD_RAZD'];
             DM.MemoryData_services['MemoryData_st_tittle']   := DM.DataSet_read['KOD_STAT'];
             DM.MemoryData_services['MemoryData_KEKV_tittle'] := kekv_kod;
             DM.MemoryData_services['MemoryData_st_options']  := DM.DataSet_read['ST_OPTIONS'];
             DM.MemoryData_services['MemoryData_date_beg']  := DM.DataSet_read['DATE_BEG'];
             DM.MemoryData_services['MemoryData_date_end']  := DM.DataSet_read['DATE_END'];
             DM.MemoryData_services.Post;
...
function Tfrmprice_param_AE.check_data:boolean;
var i, j, rec, Kol_All_Day, Kol_Day_Sum, Kol_Day:integer;
    date_beg, date_end: Tdate;
begin
  If DM.MemoryData_services.RecordCount = 0 then Exit;
  ...
  For i := 0 to DM.MemoryData_services.RecordCount - 1 do
   Begin
       date_beg:=DM.MemoryData_services['MemoryData_date_beg'];
       date_end:=DM.MemoryData_services['MemoryData_date_end'];

       DM.MemoryData_services.First;

       For j := 0 to DM.MemoryData_services.RecordCount - 1 do
       Begin
           if not( //периоды совпадают
                  ((date_beg=DM.MemoryData_services['MemoryData_date_beg'])
                   and (date_end=DM.MemoryData_services['MemoryData_date_end']))
                   or not  //периоды не пересекаются
                   ((date_end>=DM.MemoryData_services['MemoryData_date_beg'])
                   and (date_beg<=DM.MemoryData_services['MemoryData_date_end']))
                  )
            then begin result:=true; exit; end;
            DM.MemoryData_services.Next;
       end;
       DM.MemoryData_services.First;
       For j := 0 to i do DM.MemoryData_services.Next;
...
begin
  ...
  if DM.MemoryData_services.RecordCount = 0 then
   ...
   if check_data then
   begin
       ShowMessage('В одній категорії оплати періоди повинні або повністю збігатися, або не перетинатися!');
       exit;
   end;
