object ShowNNForm: TShowNNForm
  Left = 257
  Top = 317
  Width = 991
  Height = 526
  Caption = #1055#1086#1076#1072#1090#1082#1086#1074#1072' '#1085#1072#1082#1083#1072#1076#1085#1072
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poMainFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object pnl1: TPanel
    Left = 0
    Top = 0
    Width = 975
    Height = 41
    Align = alTop
    TabOrder = 0
    object lbl1: TLabel
      Left = 16
      Top = 16
      Width = 21
      Height = 13
      Caption = 'lbl1'
    end
    object cxButtonExit: TcxButton
      Left = 914
      Top = 5
      Width = 48
      Height = 28
      TabOrder = 0
      OnClick = cxButtonExitClick
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      LookAndFeel.Kind = lfFlat
      UseSystemPaint = False
    end
    object cxButton1: TcxButton
      Left = 800
      Top = 5
      Width = 102
      Height = 28
      Caption = #1044#1088#1091#1082
      TabOrder = 1
      OnClick = cxButton1Click
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      LookAndFeel.Kind = lfFlat
      UseSystemPaint = False
    end
    object cxCheckEdit: TcxCheckBox
      Left = 776
      Top = 16
      Width = 25
      Height = 21
      Properties.DisplayUnchecked = 'False'
      Properties.Caption = ''
      TabOrder = 2
    end
  end
  object cxGrid1: TcxGrid
    Left = 0
    Top = 41
    Width = 975
    Height = 447
    Align = alClient
    TabOrder = 1
    object cxGrid1DBTableView1: TcxGridDBTableView
      DataController.DataSource = ds1
      DataController.Filter.Criteria = {FFFFFFFF0000000000}
      DataController.Summary.DefaultGroupSummaryItems = <
        item
          Format = ',0.00;-,0.00'
          Kind = skSum
          Position = spFooter
          Column = cxGrid1DBTableView1DBColumn7
        end
        item
          Format = ',0.00;-,0.00'
          Kind = skSum
          Position = spFooter
          Column = cxGrid1DBTableView1DBColumn13
        end
        item
          Format = ',0.00;-,0.00'
          Kind = skSum
          Position = spFooter
          Column = cxGrid1DBTableView1DBColumn14
        end
        item
          Format = ',0.00;-,0.00'
          Kind = skSum
          Position = spFooter
          Column = cxGrid1DBTableView1DBColumn10
        end
        item
          Format = ',0.00;-,0.00'
          Kind = skSum
          Position = spFooter
          Column = cxGrid1DBTableView1DBColumn12
        end
        item
          Format = ',0.00;-,0.00'
          Kind = skSum
          Position = spFooter
          Column = cxGrid1DBTableView1DBColumn11
        end>
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0.00;-,0.00'
          Kind = skSum
          FieldName = 'S_NACH'
          Column = cxGrid1DBTableView1DBColumn7
        end>
      DataController.Summary.SummaryGroups = <>
      NavigatorButtons.ConfirmDelete = False
      OptionsSelection.CellSelect = False
      OptionsView.Footer = True
      OptionsView.GroupFooters = gfAlwaysVisible
      Styles.Background = cxStyle13
      Styles.Content = cxStyle13
      Styles.Header = cxStyle2
      object cxGrid1DBTableView1DBColumn1: TcxGridDBColumn
        Caption = #1055#1030#1041
        Options.Editing = False
        Options.Filtering = False
        SortOrder = soAscending
        Width = 220
        DataBinding.FieldName = 'FIO'
      end
      object cxGrid1DBTableView1DBColumn2: TcxGridDBColumn
        Caption = #1044#1086#1075#1086#1074#1086#1088
        Options.Editing = False
        Options.Filtering = False
        Width = 59
        DataBinding.FieldName = 'NUM_DOG'
      end
      object cxGrid1DBTableView1DBColumn3: TcxGridDBColumn
        Caption = #1041#1102#1076#1078#1077#1090
        Options.Editing = False
        Options.Filtering = False
        Width = 62
        DataBinding.FieldName = 'SM_KOD'
      end
      object cxGrid1DBTableView1DBColumn4: TcxGridDBColumn
        Caption = #1056#1086#1079#1076#1110#1083
        Options.Editing = False
        Options.Filtering = False
        Width = 49
        DataBinding.FieldName = 'RZ_KOD'
      end
      object cxGrid1DBTableView1DBColumn5: TcxGridDBColumn
        Caption = #1057#1090#1072#1090#1090#1103
        Options.Editing = False
        Options.Filtering = False
        Width = 49
        DataBinding.FieldName = 'ST_KOD'
      end
      object cxGrid1DBTableView1DBColumn6: TcxGridDBColumn
        Caption = #1050#1045#1050#1042
        Options.Editing = False
        Options.Filtering = False
        Width = 40
        DataBinding.FieldName = 'KEKV_KOD'
      end
      object cxGrid1DBTableView1DBColumn9: TcxGridDBColumn
        Caption = #1044#1072#1090#1072' '#1086#1089#1090#1072#1085#1100#1086#1075#1086' '#1088#1086#1079#1088#1072#1093#1091#1085#1082#1091
        Options.Editing = False
        Options.Filtering = False
        Width = 176
        DataBinding.FieldName = 'DATE_NN_BEFORE'
      end
      object cxGrid1DBTableView1DBColumn11: TcxGridDBColumn
        Caption = #1047#1072#1075#1072#1083#1100#1085#1072' '#1089#1091#1084#1072' '#1085#1072#1088#1072#1093#1091#1074#1072#1085#1100
        Options.Editing = False
        Options.Filtering = False
        Width = 172
        DataBinding.FieldName = 'SUMMA_NACH'
      end
      object cxGrid1DBTableView1DBColumn12: TcxGridDBColumn
        Caption = #1047#1072#1075#1072#1083#1100#1085#1072' '#1089#1091#1084#1072' '#1089#1087#1083#1072#1090
        Options.Editing = False
        Options.Filtering = False
        Width = 158
        DataBinding.FieldName = 'SUMMA_OPL'
      end
      object cxGrid1DBTableView1DBColumn10: TcxGridDBColumn
        Caption = #1047#1072#1075#1072#1083#1100#1085#1072' '#1089#1091#1084#1072' '#1086#1087#1086#1076#1072#1090#1082#1091#1074#1072#1085#1085#1103
        Options.Editing = False
        Options.Filtering = False
        Width = 188
        DataBinding.FieldName = 'S_ALL_NACH'
      end
      object cxGrid1DBTableView1DBColumn13: TcxGridDBColumn
        Caption = #1057#1091#1084#1072' '#1085#1072#1088#1072#1093#1091#1074#1072#1085#1100
        Options.Editing = False
        Options.Filtering = False
        Width = 109
        DataBinding.FieldName = 'SUMMA_NACHISL_PERIOD'
      end
      object cxGrid1DBTableView1DBColumn14: TcxGridDBColumn
        Caption = #1057#1091#1084#1072' '#1089#1087#1083#1072#1090
        Options.Editing = False
        Options.Filtering = False
        Width = 86
        DataBinding.FieldName = 'SUMMA_OPLAT_PERIOD'
      end
      object cxGrid1DBTableView1DBColumn7: TcxGridDBColumn
        Caption = #1057#1091#1084#1072' '#1086#1087#1086#1076#1072#1090#1082#1091#1074#1072#1085#1085#1103
        Options.Editing = False
        Options.Filtering = False
        Width = 113
        DataBinding.FieldName = 'S_NACH'
      end
      object cxGrid1DBTableView1DBColumn8: TcxGridDBColumn
        Caption = #1050#1086#1085#1090#1088#1072#1075#1077#1085#1090
        Visible = False
        GroupIndex = 0
        Options.Editing = False
        Options.Filtering = False
        SortOrder = soAscending
        DataBinding.FieldName = 'NAME_CUSTOMER'
      end
    end
    object cxGrid1Level1: TcxGridLevel
      GridView = cxGrid1DBTableView1
    end
  end
  object ds1: TDataSource
    DataSet = pFIBDataSet
    Left = 416
    Top = 104
  end
  object pFIBDataSet: TpFIBDataSet
    Database = NNMainForm.DatabaseWork
    Transaction = pFIBTransaction1
    SelectSQL.Strings = (
      'select * from PC_NN_DOG_SELECT(:id_reg, :date_nn)'
      'order by name_customer, fio, sm_kod, rz_kod, st_kod, kekv_kod')
    Left = 456
    Top = 104
    poSQLINT64ToBCD = True
    object pFIBDataSetID_DOG: TFIBBCDField
      FieldName = 'ID_DOG'
      Size = 0
      RoundByScale = True
    end
    object pFIBDataSetKOD_DOG: TFIBBCDField
      FieldName = 'KOD_DOG'
      Size = 0
      RoundByScale = True
    end
    object pFIBDataSetID_SM: TFIBBCDField
      FieldName = 'ID_SM'
      Size = 0
      RoundByScale = True
    end
    object pFIBDataSetID_ST: TFIBBCDField
      FieldName = 'ID_ST'
      Size = 0
      RoundByScale = True
    end
    object pFIBDataSetID_RZ: TFIBBCDField
      FieldName = 'ID_RZ'
      Size = 0
      RoundByScale = True
    end
    object pFIBDataSetID_KEKV: TFIBBCDField
      FieldName = 'ID_KEKV'
      Size = 0
      RoundByScale = True
    end
    object pFIBDataSetS_NACH: TFIBBCDField
      FieldName = 'S_NACH'
      Size = 2
      RoundByScale = True
    end
    object pFIBDataSetSM_KOD: TFIBStringField
      FieldName = 'SM_KOD'
      Size = 10
      EmptyStrToNull = True
    end
    object pFIBDataSetRZ_KOD: TFIBStringField
      FieldName = 'RZ_KOD'
      Size = 10
      EmptyStrToNull = True
    end
    object pFIBDataSetST_KOD: TFIBStringField
      FieldName = 'ST_KOD'
      Size = 10
      EmptyStrToNull = True
    end
    object pFIBDataSetKEKV_KOD: TFIBStringField
      FieldName = 'KEKV_KOD'
      Size = 10
      EmptyStrToNull = True
    end
    object pFIBDataSetFIO: TFIBStringField
      FieldName = 'FIO'
      Size = 100
      EmptyStrToNull = True
    end
    object pFIBDataSetNUM_DOG: TFIBStringField
      FieldName = 'NUM_DOG'
      EmptyStrToNull = True
    end
    object pFIBDataSetID_CUSTOMER: TFIBBCDField
      FieldName = 'ID_CUSTOMER'
      Size = 0
      RoundByScale = True
    end
    object pFIBDataSetNAME_CUSTOMER: TFIBStringField
      FieldName = 'NAME_CUSTOMER'
      Size = 180
      EmptyStrToNull = True
    end
    object pFIBDataSetSUMMA_NACHISL_PERIOD: TFIBBCDField
      FieldName = 'SUMMA_NACHISL_PERIOD'
      Size = 2
      RoundByScale = True
    end
    object pFIBDataSetSUMMA_OPLAT_PERIOD: TFIBBCDField
      FieldName = 'SUMMA_OPLAT_PERIOD'
      Size = 2
      RoundByScale = True
    end
    object pFIBDataSetSUMMA_NACH: TFIBBCDField
      FieldName = 'SUMMA_NACH'
      Size = 2
      RoundByScale = True
    end
    object pFIBDataSetSUMMA_OPL: TFIBBCDField
      FieldName = 'SUMMA_OPL'
      Size = 2
      RoundByScale = True
    end
    object pFIBDataSetS_ALL_NACH: TFIBBCDField
      FieldName = 'S_ALL_NACH'
      Size = 2
      RoundByScale = True
    end
    object pFIBDataSetDATE_NN_BEFORE: TFIBDateField
      FieldName = 'DATE_NN_BEFORE'
    end
  end
  object StyleRepository: TcxStyleRepository
    Left = 137
    Top = 123
    object cxStyle1: TcxStyle
      AssignedValues = [svColor, svFont]
      Color = 15451300
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
    end
    object cxStyle2: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object cxStyle3: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle4: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle5: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = clBtnFace
      TextColor = clBlack
    end
    object cxStyle6: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14811135
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clNavy
    end
    object cxStyle7: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = clMoneyGreen
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object cxStyle8: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = 4707838
      TextColor = clBlack
    end
    object cxStyle9: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle10: TcxStyle
      AssignedValues = [svColor, svFont]
      Color = 15451300
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
    end
    object cxStyle11: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 4707838
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle12: TcxStyle
      AssignedValues = [svColor]
      Color = 15451300
    end
    object cxStyle13: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14811135
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle14: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 12937777
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clWhite
    end
    object GridTableViewStyleSheetDevExpress: TcxGridTableViewStyleSheet
      Caption = 'DevExpress'
      Styles.Background = cxStyle1
      Styles.Content = cxStyle2
      Styles.ContentEven = cxStyle3
      Styles.ContentOdd = cxStyle4
      Styles.Inactive = cxStyle10
      Styles.IncSearch = cxStyle11
      Styles.Selection = cxStyle14
      Styles.FilterBox = cxStyle5
      Styles.Footer = cxStyle6
      Styles.Group = cxStyle7
      Styles.GroupByBox = cxStyle8
      Styles.Header = cxStyle9
      Styles.Indicator = cxStyle12
      Styles.Preview = cxStyle13
      BuiltIn = True
    end
  end
  object pFIBTransaction1: TpFIBTransaction
    DefaultDatabase = NNMainForm.DatabaseWork
    TimeoutAction = TARollback
    Left = 368
    Top = 104
  end
  object frxDesigner: TfrxDesigner
    Restrictions = []
    Left = 488
    Top = 104
  end
  object frxReport: TfrxReport
    Version = '3.15'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    ReportOptions.CreateDate = 41636.394040798610000000
    ReportOptions.LastChange = 41636.394040798610000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 520
    Top = 104
    Datasets = <>
    Variables = <>
    Style = <>
  end
  object frxDBDataset: TfrxDBDataset
    UserName = 'frxDBDataset'
    CloseDataSource = False
    DataSet = pFIBDataSet
    Left = 488
    Top = 136
  end
  object actlst1: TActionList
    Left = 320
    Top = 120
    object act1: TAction
      Caption = #1044#1080#1079#1072#1081#1085#1077#1088' '#1086#1090#1095#1077#1090#1086#1074
      ShortCut = 57412
      OnExecute = act1Execute
    end
  end
  object frxRTFExport1: TfrxRTFExport
    UseFileCache = True
    ShowProgress = True
    Wysiwyg = True
    Creator = 'FastReport http://www.fast-report.com'
    Left = 80
    Top = 136
  end
  object frxXLSExport1: TfrxXLSExport
    UseFileCache = True
    ShowProgress = True
    AsText = False
    Background = True
    FastExport = True
    PageBreaks = True
    Left = 80
    Top = 192
  end
  object frxPDFExport1: TfrxPDFExport
    UseFileCache = True
    PrintOptimized = False
    Outline = False
    Author = 'FastReport'#174
    Subject = 'FastReport'#174' PDF export'
    Background = False
    Creator = 'FastReport'#174' (http://www.fast-report.com)'
    HTMLTags = True
    Left = 128
    Top = 208
  end
end
