...
begin
 global_filtr:=global_filtr + prefix+ ' UPPER(FIO) LIKE '+''''+AnsiUpperCase(TextFIO.text)+'%'+''' ';
 prefix:=' AND ';
end;
...
if TextFIO.text<>''then
begin
 DataSetForGrid.SelectSQL.Add(prefix+' UPPER(FIO) LIKE '+''''+AnsiUpperCase(TextFIO.text)+'%'+''' ');
 prefix:=' AND ';
end;
...
if TextFIO.text<>''then
begin
 ds_for_grid_2.SelectSQL.Add(prefix+'UPPER(FIO) LIKE '+''''+AnsiUpperCase(TextFIO.text)+'%'+''' ');
 prefix:=' AND ';
end;
...
//  DataSetReport.SelectSQL.Text:=DataSetForGrid.SelectSQL.Text;
  ...
  if TextFIO.text<>''then
  begin
    DataSetReport.SelectSQL.Add(prefix+' is_summa in(0,1) and UPPER(FIO) LIKE '+''''+AnsiUpperCase(TextFIO.text)+'%'+''' ');
    prefix:=' AND ';
    ex_filtr:=1;
  end;
 ...
 begin
  ...
  if TextFIO.text<>''then
  begin
    DataSetReport.SelectSQL.Add(prefix+' UPPER(FIO) LIKE '+''''+AnsiUpperCase(TextFIO.text)+'%'+''' ');
    prefix:=' AND ';
  end;
