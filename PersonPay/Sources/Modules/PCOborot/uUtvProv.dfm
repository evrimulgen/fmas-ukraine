object UtvProvForm: TUtvProvForm
  Left = 303
  Top = 346
  Width = 793
  Height = 428
  Caption = #1047#1072#1090#1074#1077#1088#1076#1078#1077#1085#1085#1103' '#1087#1088#1086#1074#1086#1076#1086#1082' '#1076#1086#1082#1091#1084#1077#1085#1090#1072
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poMainFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object lblDoc: TLabel
    Left = 22
    Top = 22
    Width = 60
    Height = 13
    Caption = #1044#1086#1082#1091#1084#1077#1085#1090
  end
  object lbl2: TLabel
    Left = 25
    Top = 48
    Width = 45
    Height = 13
    Caption = #1042#1072#1083#1102#1090#1072
  end
  object lbl3: TLabel
    Left = 25
    Top = 78
    Width = 88
    Height = 13
    Caption = #1055#1086#1090#1086#1095#1085#1080#1081' '#1082#1091#1088#1089
  end
  object lbl4: TLabel
    Left = 346
    Top = 110
    Width = 161
    Height = 13
    Caption = #1057#1091#1084#1072' '#1079#1072' '#1076#1086#1082#1091#1084#1077#1085#1090#1086#1084' ('#1075#1088#1085'.)'
  end
  object lbl5: TLabel
    Left = 26
    Top = 110
    Width = 162
    Height = 13
    Caption = #1057#1091#1084#1072' '#1079#1072' '#1076#1086#1082#1091#1084#1077#1085#1090#1086#1084' ('#1074#1072#1083'.)'
  end
  object lbl6: TLabel
    Left = 353
    Top = 78
    Width = 165
    Height = 13
    Caption = #1044#1072#1090#1072'/'#1095#1072#1089' '#1088#1086#1079#1088#1072#1093#1091#1085#1082#1091' '#1082#1091#1088#1089#1091
  end
  object pnl1: TPanel
    Left = 0
    Top = 344
    Width = 777
    Height = 46
    Align = alBottom
    TabOrder = 0
    object cxOk: TcxButton
      Left = 442
      Top = 10
      Width = 146
      Height = 25
      Caption = #1055#1110#1076#1090#1074#1077#1088#1076#1080#1090#1080
      TabOrder = 0
      OnClick = cxOkClick
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      UseSystemPaint = False
    end
    object cxClose: TcxButton
      Left = 621
      Top = 10
      Width = 146
      Height = 25
      Caption = #1042#1110#1076#1084#1110#1085#1080#1090#1080
      TabOrder = 1
      OnClick = cxCloseClick
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      UseSystemPaint = False
    end
  end
  object pnl2: TPanel
    Left = 0
    Top = 140
    Width = 777
    Height = 204
    Align = alBottom
    TabOrder = 1
    object cxGrid1: TcxGrid
      Left = 1
      Top = 1
      Width = 775
      Height = 202
      Align = alClient
      TabOrder = 0
      object cxGrid1DBTableView1: TcxGridDBTableView
        DataController.DataSource = ds1
        DataController.Filter.Criteria = {FFFFFFFF0000000000}
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <
          item
            Format = ',0.00;-,0.00'
            Kind = skSum
            FieldName = 'SUMMA_VAL'
            Column = cxgrdbclmnGrid1DBTableView1DBColumn6
          end
          item
            Format = ',0.00;-,0.00'
            Kind = skSum
            FieldName = 'SUMMA_GRN'
            Column = cxgrdbclmnGrid1DBTableView1DBColumn5
          end
          item
            Format = ',0.00;-,0.00'
            Kind = skCount
            FieldName = 'ID_PROV'
            Column = cxgrdbclmnFIO
          end>
        DataController.Summary.SummaryGroups = <>
        NavigatorButtons.ConfirmDelete = False
        OptionsView.Footer = True
        OptionsView.GroupByBox = False
        Styles.Background = cxStyle13
        Styles.Content = cxStyle13
        Styles.Header = cxStyle2
        object cxgrdbclmnFIO: TcxGridDBColumn
          Caption = #1055#1030#1041
          Options.Editing = False
          Options.Filtering = False
          Width = 189
          DataBinding.FieldName = 'FIO'
        end
        object cxgrdbclmnNUM_DOG: TcxGridDBColumn
          Caption = #1044#1086#1075#1086#1074#1110#1088
          Options.Editing = False
          Options.Filtering = False
          Width = 64
          DataBinding.FieldName = 'NUM_DOG'
        end
        object cxgrdbclmnNUM_SCH_DB: TcxGridDBColumn
          Caption = #1044#1073'.'#1088#1072#1093'.'
          Options.Editing = False
          Options.Filtering = False
          Width = 59
          DataBinding.FieldName = 'NUM_SCH_DB'
        end
        object cxgrdbclmnNUM_SCH_KR: TcxGridDBColumn
          Caption = #1050#1088'.'#1088#1072#1093'.'
          Options.Editing = False
          Options.Filtering = False
          Width = 52
          DataBinding.FieldName = 'NUM_SCH_KR'
        end
        object cxgrdbclmnGrid1DBTableView1DBColumn1: TcxGridDBColumn
          Caption = #1041#1102#1076#1078#1077#1090
          Options.Editing = False
          Options.Filtering = False
          Width = 51
          DataBinding.FieldName = 'SM_KOD'
        end
        object cxgrdbclmnGrid1DBTableView1DBColumn2: TcxGridDBColumn
          Caption = #1056#1086#1079#1076#1110#1083
          Options.Editing = False
          Options.Filtering = False
          Width = 45
          DataBinding.FieldName = 'RZ_KOD'
        end
        object cxgrdbclmnGrid1DBTableView1DBColumn3: TcxGridDBColumn
          Caption = #1057#1090#1072#1090#1090#1103
          Options.Editing = False
          Options.Filtering = False
          Width = 46
          DataBinding.FieldName = 'ST_KOD'
        end
        object cxgrdbclmnGrid1DBTableView1DBColumn4: TcxGridDBColumn
          Caption = #1050#1045#1050#1042
          Options.Editing = False
          Options.Filtering = False
          Width = 41
          DataBinding.FieldName = 'KEKV_KOD'
        end
        object cxgrdbclmnGrid1DBTableView1DBColumn5: TcxGridDBColumn
          Caption = #1057#1091#1084#1072' ('#1075#1088#1085'.)'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = '0.00;-0.00'
          Options.Editing = False
          Options.Filtering = False
          SortOrder = soAscending
          Width = 117
          DataBinding.FieldName = 'SUMMA_GRN'
        end
        object cxgrdbclmnGrid1DBTableView1DBColumn6: TcxGridDBColumn
          Caption = #1057#1091#1084#1072'('#1074#1072#1083'.)'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.00;-,0.00'
          Properties.Nullable = False
          Properties.ReadOnly = False
          Properties.UseLeftAlignmentOnEditing = False
          Options.Filtering = False
          Width = 104
          DataBinding.FieldName = 'SUMMA_VAL'
        end
      end
      object cxGrid1Level1: TcxGridLevel
        GridView = cxGrid1DBTableView1
      end
    end
  end
  object cxKurs: TcxCurrencyEdit
    Left = 215
    Top = 75
    Width = 121
    Height = 21
    Properties.DecimalPlaces = 8
    Properties.DisplayFormat = '0.00000000'
    TabOrder = 2
    OnExit = cxKursExit
  end
  object cxSumGrn: TcxCurrencyEdit
    Left = 535
    Top = 110
    Width = 121
    Height = 21
    Properties.Alignment.Horz = taRightJustify
    Properties.DisplayFormat = ',0.00;-,0.00'
    Properties.ReadOnly = True
    Properties.UseLeftAlignmentOnEditing = False
    TabOrder = 3
  end
  object cxSumVal: TcxCurrencyEdit
    Left = 215
    Top = 110
    Width = 121
    Height = 21
    Properties.Alignment.Horz = taRightJustify
    Properties.DisplayFormat = ',0.00;-,0.00'
    Properties.ReadOnly = True
    Properties.UseLeftAlignmentOnEditing = False
    TabOrder = 4
  end
  object lbl1: TcxTextEdit
    Left = 109
    Top = 18
    Width = 557
    Height = 21
    TabOrder = 5
  end
  object cxValute: TcxButtonEdit
    Left = 109
    Top = 45
    Width = 311
    Height = 21
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end>
    Properties.OnButtonClick = cxValutePropertiesButtonClick
    TabOrder = 6
  end
  object cxDateTime: TcxDateEdit
    Left = 520
    Top = 75
    Width = 89
    Height = 21
    Properties.ShowTime = False
    TabOrder = 7
    OnExit = cxDateTimeExit
    EditValue = 0d
  end
  object cxTimeEdit: TcxTimeEdit
    Left = 610
    Top = 75
    Width = 73
    Height = 21
    EditValue = 0.000000000000000000
    TabOrder = 8
    OnExit = cxTimeEditExit
  end
  object StyleRepository: TcxStyleRepository
    Left = 227
    Top = 208
    object cxStyle1: TcxStyle
      AssignedValues = [svColor, svFont]
      Color = 15451300
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
    end
    object cxStyle2: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object cxStyle3: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle4: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle5: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = clBtnFace
      TextColor = clBlack
    end
    object cxStyle6: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 2191320
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clNavy
    end
    object cxStyle7: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = clMoneyGreen
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object cxStyle8: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = 4707838
      TextColor = clBlack
    end
    object cxStyle9: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle10: TcxStyle
      AssignedValues = [svColor, svFont]
      Color = 15451300
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
    end
    object cxStyle11: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 4707838
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle12: TcxStyle
      AssignedValues = [svColor]
      Color = 15451300
    end
    object cxStyle13: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14811135
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle14: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 12937777
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clWhite
    end
    object GridTableViewStyleSheetDevExpress: TcxGridTableViewStyleSheet
      Caption = 'DevExpress'
      Styles.Background = cxStyle1
      Styles.Content = cxStyle2
      Styles.ContentEven = cxStyle3
      Styles.ContentOdd = cxStyle4
      Styles.Inactive = cxStyle10
      Styles.IncSearch = cxStyle11
      Styles.Selection = cxStyle14
      Styles.FilterBox = cxStyle5
      Styles.Footer = cxStyle6
      Styles.Group = cxStyle7
      Styles.GroupByBox = cxStyle8
      Styles.Header = cxStyle9
      Styles.Indicator = cxStyle12
      Styles.Preview = cxStyle13
      BuiltIn = True
    end
  end
  object ds1: TDataSource
    DataSet = RxMemoryData
    Left = 155
    Top = 211
  end
  object ST_PR: TpFIBStoredProc
    Left = 121
    Top = 211
  end
  object TR: TpFIBTransaction
    DefaultDatabase = DB
    TimeoutAction = TARollback
    Left = 90
    Top = 209
  end
  object DS: TpFIBDataSet
    Database = DB
    Transaction = TR
    SelectSQL.Strings = (
      
        'select * from PC_DT_PROV_TMP_INFO_SEL(:id_doc, :id_session, :dat' +
        '_reg)')
    OnCalcFields = DSCalcFields
    Left = 47
    Top = 210
    poSQLINT64ToBCD = True
    object DSID_SM: TFIBBCDField
      FieldName = 'ID_SM'
      Size = 0
      RoundByScale = True
    end
    object DSID_RZ: TFIBBCDField
      FieldName = 'ID_RZ'
      Size = 0
      RoundByScale = True
    end
    object DSID_ST: TFIBBCDField
      FieldName = 'ID_ST'
      Size = 0
      RoundByScale = True
    end
    object DSID_KEKV: TFIBBCDField
      FieldName = 'ID_KEKV'
      Size = 0
      RoundByScale = True
    end
    object DSSM_TITLE: TFIBStringField
      FieldName = 'SM_TITLE'
      Size = 200
      EmptyStrToNull = True
    end
    object DSSM_KOD: TFIBIntegerField
      FieldName = 'SM_KOD'
    end
    object DSRZ_TITLE: TFIBStringField
      FieldName = 'RZ_TITLE'
      Size = 200
      EmptyStrToNull = True
    end
    object DSRZ_KOD: TFIBIntegerField
      FieldName = 'RZ_KOD'
    end
    object DSST_TITLE: TFIBStringField
      FieldName = 'ST_TITLE'
      Size = 200
      EmptyStrToNull = True
    end
    object DSST_KOD: TFIBIntegerField
      FieldName = 'ST_KOD'
    end
    object DSKEKV_TITLE: TFIBStringField
      FieldName = 'KEKV_TITLE'
      Size = 200
      EmptyStrToNull = True
    end
    object DSKEKV_KOD: TFIBIntegerField
      FieldName = 'KEKV_KOD'
    end
    object DSID_PROV: TFIBBCDField
      FieldName = 'ID_PROV'
      Size = 0
      RoundByScale = True
    end
    object DSSUMMA_GRN: TFIBBCDField
      FieldName = 'SUMMA_GRN'
      DisplayFormat = ',0.00;-,0.00'
      Size = 2
      RoundByScale = True
    end
    object DSSUMMA_VAL: TFIBBCDField
      FieldName = 'SUMMA_VAL'
      OnGetText = DSSUMMA_VALGetText
      DisplayFormat = ',0.00;-,0.00'
      Size = 2
      RoundByScale = True
    end
    object DSSumma_val_: TCurrencyField
      FieldKind = fkCalculated
      FieldName = 'Summa_val_'
      DisplayFormat = '##.##'
      Calculated = True
    end
    object DSID_SCH_DB: TFIBIntegerField
      FieldName = 'ID_SCH_DB'
    end
    object DSID_SCH_KR: TFIBIntegerField
      FieldName = 'ID_SCH_KR'
    end
    object DSNUM_SCH_DB: TFIBStringField
      FieldName = 'NUM_SCH_DB'
      Size = 30
      EmptyStrToNull = True
    end
    object DSNUM_SCH_KR: TFIBStringField
      FieldName = 'NUM_SCH_KR'
      Size = 30
      EmptyStrToNull = True
    end
    object DSNAME_SCH_DB: TFIBStringField
      FieldName = 'NAME_SCH_DB'
      Size = 60
      EmptyStrToNull = True
    end
    object DSNAME_SCH_KR: TFIBStringField
      FieldName = 'NAME_SCH_KR'
      Size = 60
      EmptyStrToNull = True
    end
    object DSID_OPER: TFIBBCDField
      FieldName = 'ID_OPER'
      Size = 0
      RoundByScale = True
    end
    object DSFIO: TFIBStringField
      FieldName = 'FIO'
      Size = 500
      EmptyStrToNull = True
    end
    object DSNUM_DOG: TFIBStringField
      FieldName = 'NUM_DOG'
      Size = 75
      EmptyStrToNull = True
    end
    object DSSYMBOL: TFIBStringField
      FieldName = 'SYMBOL'
      Size = 3
      EmptyStrToNull = True
    end
  end
  object DB: TpFIBDatabase
    DBParams.Strings = (
      'user_name=SYSDBA'
      'password=masterkey'
      'lc_ctype=WIN1251')
    DefaultTransaction = TR
    SQLDialect = 3
    Timeout = 0
    WaitForRestoreConnect = 0
    Left = 5
    Top = 208
  end
  object DS_DOC: TpFIBDataSet
    Database = DB
    Transaction = TR
    SelectSQL.Strings = (
      'select * from PC_DT_DOC_TMP_SEL_INFO(:id_doc, :id_session)')
    Left = 45
    Top = 241
    poSQLINT64ToBCD = True
    object DS_DOCNUM_DOC: TFIBStringField
      FieldName = 'NUM_DOC'
      Size = 120
      EmptyStrToNull = True
    end
    object DS_DOCDATE_DOC: TFIBDateField
      FieldName = 'DATE_DOC'
    end
    object DS_DOCTYPE_DOC: TFIBStringField
      FieldName = 'TYPE_DOC'
      Size = 25
      EmptyStrToNull = True
    end
    object DS_DOCDB_SUMMA: TFIBBCDField
      FieldName = 'DB_SUMMA'
      EditFormat = '##.##'
      Size = 2
      RoundByScale = True
    end
    object DS_DOCKR_SUMMA: TFIBBCDField
      FieldName = 'KR_SUMMA'
      EditFormat = '##.##'
      Size = 2
      RoundByScale = True
    end
    object DS_DOCDB_SUMMA_VAL: TFIBBCDField
      FieldName = 'DB_SUMMA_VAL'
      EditFormat = '##.##'
      Size = 2
      RoundByScale = True
    end
    object DS_DOCKR_SUMMA_VAL: TFIBBCDField
      FieldName = 'KR_SUMMA_VAL'
      EditFormat = '##.##'
      Size = 2
      RoundByScale = True
    end
    object DS_DOCNOTE: TFIBStringField
      FieldName = 'NOTE'
      Size = 255
      EmptyStrToNull = True
    end
    object DS_DOCID_TYPE_DOC: TFIBBCDField
      FieldName = 'ID_TYPE_DOC'
      Size = 0
      RoundByScale = True
    end
    object DS_DOCIS_OUR_DOC: TFIBIntegerField
      FieldName = 'IS_OUR_DOC'
    end
    object DS_DOCID_VALUTE: TFIBBCDField
      FieldName = 'ID_VALUTE'
      Size = 0
      RoundByScale = True
    end
    object DS_DOCKURS: TFIBBCDField
      FieldName = 'KURS'
      RoundByScale = True
    end
    object DS_DOCIS_UTV: TFIBIntegerField
      FieldName = 'IS_UTV'
    end
    object DS_DOCIS_DB_KR: TFIBIntegerField
      FieldName = 'IS_DB_KR'
    end
    object DS_DOCN_VAL: TFIBStringField
      FieldName = 'N_VAL'
      Size = 5
      EmptyStrToNull = True
    end
    object DS_DOCVAL_TITLE: TFIBStringField
      FieldName = 'VAL_TITLE'
      Size = 100
      EmptyStrToNull = True
    end
    object DS_DOCDATE_TIME_OPLAT: TFIBDateTimeField
      FieldName = 'DATE_TIME_OPLAT'
    end
  end
  object actlst1: TActionList
    Left = 371
    Top = 136
    object actRefresh: TAction
      Caption = 'actRefresh'
      OnExecute = actRefreshExecute
    end
    object actSave: TAction
      Caption = 'actSave'
      ShortCut = 121
      OnExecute = actSaveExecute
    end
    object actReWrite: TAction
      Caption = 'actReWrite'
      OnExecute = actReWriteExecute
    end
  end
  object StPrKurs: TpFIBStoredProc
    Database = DB
    Transaction = TR
    Left = 119
    Top = 309
  end
  object StPrKurs1: TpFIBStoredProc
    Database = DB
    Transaction = TR
    Left = 154
    Top = 311
  end
  object tr_kurs: TpFIBTransaction
    DefaultDatabase = DB
    TimeoutAction = TARollback
    Left = 120
    Top = 344
  end
  object tr_kurs1: TpFIBTransaction
    DefaultDatabase = DB
    TimeoutAction = TARollback
    Left = 157
    Top = 345
  end
  object DS_D: TpFIBDataSet
    Database = DB
    Transaction = TR
    SelectSQL.Strings = (
      
        'select * from PC_PUB_DOC a inner join pub_dt_all_doc b on a.pk_i' +
        'd=b.pk_id where a.pc_id_doc=:id_doc')
    Left = 83
    Top = 309
    poSQLINT64ToBCD = True
    object DS_DPC_ID_DOC: TFIBBCDField
      FieldName = 'PC_ID_DOC'
      Size = 0
      RoundByScale = True
    end
    object DS_DPUB_ID_DOC: TFIBBCDField
      FieldName = 'PUB_ID_DOC'
      Size = 0
      RoundByScale = True
    end
    object DS_DPK_ID: TFIBBCDField
      FieldName = 'PK_ID'
      Size = 0
      RoundByScale = True
    end
    object DS_DPUB_DATE_DOC: TFIBDateField
      FieldName = 'PUB_DATE_DOC'
    end
    object DS_DSUMMA_GRN: TFIBBCDField
      FieldName = 'SUMMA_GRN'
      Size = 4
      RoundByScale = True
    end
    object DS_DSUMMA_VAL: TFIBBCDField
      FieldName = 'SUMMA_VAL'
      Size = 4
      RoundByScale = True
    end
    object DS_DID_VALYUTA: TFIBIntegerField
      FieldName = 'ID_VALYUTA'
    end
    object DS_DKURS: TFIBBCDField
      FieldName = 'KURS'
      RoundByScale = True
    end
    object DS_DIS_UTV: TFIBIntegerField
      FieldName = 'IS_UTV'
    end
    object DS_DIS_DB_KR: TFIBIntegerField
      FieldName = 'IS_DB_KR'
    end
    object DS_DPK_ID1: TFIBBCDField
      FieldName = 'PK_ID1'
      Size = 0
      RoundByScale = True
    end
    object DS_DID_DOC: TFIBBCDField
      FieldName = 'ID_DOC'
      Size = 0
      RoundByScale = True
    end
    object DS_DDATETIME: TFIBDateTimeField
      FieldName = 'DATETIME'
    end
    object DS_DDATE_REG: TFIBDateField
      FieldName = 'DATE_REG'
    end
    object DS_DDATE_PROV: TFIBDateField
      FieldName = 'DATE_PROV'
    end
    object DS_DDATE_DOC: TFIBDateField
      FieldName = 'DATE_DOC'
    end
    object DS_DID_TYPE_DOC: TFIBBCDField
      FieldName = 'ID_TYPE_DOC'
      Size = 0
      RoundByScale = True
    end
    object DS_DNUM_DOC: TFIBStringField
      FieldName = 'NUM_DOC'
      Size = 120
      EmptyStrToNull = True
    end
    object DS_DSUMMA: TFIBBCDField
      FieldName = 'SUMMA'
      Size = 2
      RoundByScale = True
    end
    object DS_DID_CUSTOMER: TFIBBCDField
      FieldName = 'ID_CUSTOMER'
      Size = 0
      RoundByScale = True
    end
    object DS_DNOTE: TFIBStringField
      FieldName = 'NOTE'
      Size = 255
      EmptyStrToNull = True
    end
    object DS_DFIO: TFIBStringField
      FieldName = 'FIO'
      Size = 100
      EmptyStrToNull = True
    end
    object DS_DID_RATE_ACC: TFIBBCDField
      FieldName = 'ID_RATE_ACC'
      Size = 0
      RoundByScale = True
    end
    object DS_DID_RATE_ACC_NATIVE: TFIBBCDField
      FieldName = 'ID_RATE_ACC_NATIVE'
      Size = 0
      RoundByScale = True
    end
    object DS_DPRIHOD: TFIBBooleanField
      FieldName = 'PRIHOD'
    end
    object DS_DID_USER: TFIBBCDField
      FieldName = 'ID_USER'
      Size = 0
      RoundByScale = True
    end
    object DS_DID_OUT_DOC: TFIBBCDField
      FieldName = 'ID_OUT_DOC'
      Size = 0
      RoundByScale = True
    end
    object DS_DIS_SPRAVKA: TFIBBooleanField
      FieldName = 'IS_SPRAVKA'
    end
    object DS_DID_SERVER: TFIBBCDField
      FieldName = 'ID_SERVER'
      Size = 0
      RoundByScale = True
    end
    object DS_DCOMPUTER: TFIBStringField
      FieldName = 'COMPUTER'
      Size = 255
      EmptyStrToNull = True
    end
  end
  object ds_val: TpFIBDataSet
    Database = DB
    Transaction = TR
    Left = 392
    Top = 200
    poSQLINT64ToBCD = True
  end
  object RxMemoryData: TRxMemoryData
    FieldDefs = <
      item
        Name = 'ID_SM'
        DataType = ftInteger
      end
      item
        Name = 'ID_RZ'
        DataType = ftInteger
      end
      item
        Name = 'ID_ST'
        DataType = ftInteger
      end
      item
        Name = 'ID_KEKV'
        DataType = ftInteger
      end
      item
        Name = 'SM_KOD'
        DataType = ftInteger
      end
      item
        Name = 'RZ_KOD'
        DataType = ftInteger
      end
      item
        Name = 'ST_KOD'
        DataType = ftInteger
      end
      item
        Name = 'KEKV_KOD'
        DataType = ftInteger
      end
      item
        Name = 'ID_PROV'
        DataType = ftInteger
      end
      item
        Name = 'SUMMA_GRN'
        DataType = ftFloat
      end
      item
        Name = 'SUMMA_VAL'
        DataType = ftFloat
      end
      item
        Name = 'ID_SCH_DB'
        DataType = ftInteger
      end
      item
        Name = 'ID_SCH_KR'
        DataType = ftInteger
      end
      item
        Name = 'NUM_SCH_DB'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'NUM_SCH_KR'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'ID_OPER'
        DataType = ftInteger
      end
      item
        Name = 'FIO'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'NUM_DOG'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'SYMBOL'
        DataType = ftString
        Size = 20
      end>
    Left = 449
    Top = 201
  end
  object ds_kurs: TpFIBDataSet
    Database = DB
    Transaction = TR
    Left = 368
    Top = 244
    poSQLINT64ToBCD = True
  end
end
